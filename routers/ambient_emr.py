...
def _ambient_context(
    ...
    *,
    ...
    context = _base_context(
        ...
        "ambient_scribe",
        {
            "system_type": _consultation_system_for_doctor(doctor),
...
async def ambient_start_session(
    ...
    try:
        ...
        logger.exception("Ambient EMR session start failed for doctor_id=%s", doctor.id)
        raise HTTPException(status_code=500, detail=f"Unable to start ambient session: {exc}") from exc
...
async def ambient_process_segment(
    ...
    try:
        ...
        logger.exception("Ambient EMR segment processing failed for session_id=%s doctor_id=%s", session_id, doctor.id)
        raise HTTPException(status_code=500, detail=f"Ambient transcription failed: {exc}") from exc
...
async def ambient_finalize_session(session_id: str, doctor: Doctor = Depends(get_current_doctor)):
    try:
        ...
        logger.exception("Ambient EMR finalization failed for session_id=%s doctor_id=%s", session_id, doctor.id)
        raise HTTPException(status_code=500, detail=f"Unable to finalize ambient session: {exc}") from exc
...
async def ambient_save_emr(
    ...
    try:
        ...
        logger.exception("Ambient EMR save failed for doctor_id=%s", doctor.id)
        raise HTTPException(status_code=500, detail=f"Unable to save ambient EMR: {exc}") from exc
