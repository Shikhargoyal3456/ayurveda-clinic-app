...
function upsertEnvValue(filePath, key, value) {
  ...
  if (!found) {
    ...
    updated.push('# Public media URL for Kash AI WhatsApp logo');
...
async function uploadLogo() {
  ...
  body.set('file', new Blob([imageBuffer], { type: 'image/png' }), 'kash-ai-logo.png');
  ...
  if (!response.ok || !text.startsWith('https://')) {
    throw new Error(`0x0 upload failed: ${text}`);
  }
